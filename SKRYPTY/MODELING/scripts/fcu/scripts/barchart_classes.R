barchart_classes <- function(dataset,time_feature='datestamp',feature,time_frame='month',prop=T){
  ...
  if(is.numeric(dataset[,feature])){
    ...
    dataset[,feature]=quantcut(dataset[,feature],q=5, na.rm=FALSE)
  }else{
  ...
  if(prop==T){ test_1=prop.table(test_1,margin = 1) }
  ...
  if(is.numeric(dataset[,feature])){
    ...
    colnames(test_1)=c("Date", lapply(colnames(test_1[-1]), substring, 6))# unique(dataset[,feature]))
  }
