...
load("data/lion_king_monitoring_data_20200506.RData")

### merge data for monitoring
scored_df <- read_excel("data/scoring_output/scored_2020-05-13_merged.xlsx")
...
# trim scored data
scored_df_all <- scored_df[, colnames(wf_to_score_final)]
scored_df_selected <- scored_df[, colnames(wf_to_score_final_selected_features)]
scored_df_monitoring <- scored_df[, colnames(monitoring_dataset)]
scored_df_a <- scored_df[, colnames(a)]

# merge data with the latest batch
wf_all_merged = rbind(wf_to_score_final, scored_df_all)
wf_selected_merged = rbind(wf_to_score_final_selected_features, scored_df_selected)
monitoring_merged = rbind(monitoring_dataset, scored_df_monitoring)
a_merged = rbind(a, scored_df_a)

# install last cut off file
last_cut_off <- read_excel("data/wf/13052020 cut off.xlsx")

save(list = ls(all.names = TRUE),
     file = "data/lion_king_monitoring_data_20200513.RData")
...
# select cases sent to qc from final df
...
stats <- merge(count_cases_not_comp,
               count_cases_completed,
               by.x="as.Date(BackupTime)",
               by.y="as.Date(DRCompletedDate)")
