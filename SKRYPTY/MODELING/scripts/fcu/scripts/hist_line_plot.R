hist_line_plot = function(developement_dataset, monitoring_dataset, threshold, type, time_frame='days'){
  if (time_frame == 'developement_vs_monitoring') {
    ...
    if (type == "model_performance") {
      ...
      barchart_data_developement_dataset = developement_dataset %>% group_by(datestamp)%>%
        summarize(no_cases=n())
      barchart_data_monitoring_dataset = monitoring_dataset %>% group_by(datestamp)%>%
        summarize(no_cases=n())
      
      max_error_developement_dataset = developement_dataset%>% group_by(datestamp) %>%
        summarize(max_error_developement_dataset = sum(Label))
      max_error_monitoring_dataset = monitoring_dataset %>% group_by(datestamp) %>%
      ...
      developement_dataset = developement_dataset[order(developement_dataset$scored_df_pqc, decreasing = T),]
      ...
      monitoring_dataset = monitoring_dataset[order(monitoring_dataset$scored_df_pqc, decreasing = T),]
      monitoring_dataset = monitoring_dataset[1:(which(
        abs(monitoring_dataset$scored_df_pqc-threshold_score)==min(
          abs(monitoring_dataset$scored_df_pqc-threshold_score)))), ]
      
      barchart_data_developement_dataset_selected = developement_dataset %>% group_by(datestamp) %>%
        summarize(no_cases_selected=n())
      
      barchart_data_monitoring_dataset_selected = monitoring_dataset %>% group_by(datestamp) %>%
        summarize(no_cases_selected=n())
      ...
      developement_dataset = developement_dataset %>% group_by(datestamp) %>%
      ...
      results = plot_ly() %>%
        add_trace(x = developement_dataset$datestamp,
        ...
        add_trace(x = selected_line_developement_dataset$datestamp,
        ...
        add_trace(x = selected_line_monitoring_dataset$datestamp,
        ...
        layout(# title=paste("Changes of model performance within the time.",sep = ""),
               legend = list(y = -0.05,
      ...
      results = plot_ly() %>%
        ...
        layout(# title = paste("Changes of model performance within the time.", sep = ""),
               ...
               yaxis = list(side = 'left',
                            overlaying = "y2",
                            ticksuffix = "%",
                            title = paste('Percent of errors found in ', 100*threshold, '% of population'),
                            showgrid = T,
                            zeroline = FALSE),
               yaxis2 = list(side = 'right',
                             title = 'No cases',
      ...
      developement_dataset_0 = developement_dataset[which(developement_dataset$Label == 0),] %>% group_by(datestamp) %>%
        summarize(average_score=mean(scored_df_pqc))
      developement_dataset_1 = developement_dataset[which(developement_dataset$Label == 1),] %>% group_by(datestamp) %>%
        summarize(average_score=mean(scored_df_pqc))
      
      monitoring_dataset_0 = monitoring_dataset[which(monitoring_dataset$Label == 0),] %>% group_by(datestamp) %>%
        summarize(average_score=mean(scored_df_pqc))
      monitoring_dataset_1 = monitoring_dataset[which(monitoring_dataset$Label == 1),] %>% group_by(datestamp)%>%
        summarize(average_score=mean(scored_df_pqc))
      ...
      results = plot_ly(x = developement_dataset_0$datestamp,
        ...
        add_trace(x = developement_dataset_1$datestamp,
        ...
        add_trace(x = monitoring_dataset_0$datestamp,
        ...
        add_trace(x = monitoring_dataset_1$datestamp,
