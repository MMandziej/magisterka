...
nip_list = pandas.read_sql_query(selectQuery, db_connection)['Company code'].tolist()

### Count addresses
print('Found {0} addresses'.format(len(addressList)))
cnt = 0

for nip, address in zip(nip_list, addressList):
    cnt = cnt + 1
    lat,lng = geocode_address(address)
    updateCommand = updateQuery.format(lat, lng, nip)
    if write2db == 1:
